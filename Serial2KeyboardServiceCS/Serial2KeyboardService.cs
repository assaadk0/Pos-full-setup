            ...
            private void Run()
            {
                ...
                while (!StopRequested)
                {
                    try
                    {
                        ...
                        port.Open();
                        Logger.Log(string.Format("Successfully opened serial port {0} at {1} baud.", PortName, BaudRate));
    ...
    public static class Program
    {
        public static void Main(string[] args)
        {
            ...
            if (args.Length > 0 && args[0] == "--agent")
            {
                ...
                bool createdNew;
                using (Mutex mutex = new Mutex(true, "Global\\Serial2KeyboardAgentMutex", out createdNew))
                {
                    if (!createdNew)
                    {
